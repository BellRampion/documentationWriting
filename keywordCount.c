...
//Program to count the occurrences of keywords for the Lifeloop logs. Restarts at '==' (comments) and prints out the count after the "==End" section-ender string is found.
#include <stdio.h>
#include <string.h>
...
int main(int argc, char *argv[]){
    ...
    {
        printf("Usage: ./keywordCount inputFilename outputFilename\nIf you do not supply an output filename, the output will go to standard output.\nReads through a logfile for Lifeloop and prints out total for each keyword in the logfile. Appends to output file.\n");
        return 0;
    }
    ...
    for (int iter = 0; iter < MAXLENGTH && (fgets(fileline, MAXLENGTH, input) != NULL); iter++){
        ...
        //If the substring is null, then it's not possible for this to be true
        else
        {
            //If it's the end of a section
            match = strstr(subStr, endSection);
            if (match != NULL)
            {
                //Print substring
                fprintf(output, "%s\n", subStr);
                //Print number of occurrences of each keyword
                for (int j = 0; j < NUMKEYWORDS && (instances[j].searchTerm != NULL); j++){
                    if (instances[j].count > 0)
                    {
                        //Print and reset
                        fprintf(output, "%s: %i\n", instances[j].searchTerm, instances[j].count);
                        instances[j].count = 0;
                    }
                }
                //Print and reset
